...
<template>
  ...
  <UPageBody>
    ...
    <UAlert
      v-if="loadingError" color="error" variant="subtle" icon="i-lucide-alert-circle" class="mb-6"
      :title="String(loadingError)"
    />

    <div v-for="league in leagues" :key="league.title" class="mb-8 space-y-4">
      <h2
        v-if="league.promoted.length || league.promotionPlayoff.length || league.relegated.length || league.relegationPlayoff.length"
      ...
      <div v-if="league.promoted.length" class="space-y-2">
        ...
        <LazyUPageGrid>
          <TeamCard
            v-for="(row, index) in league.promoted" :key="getRowKey('p', row, index)" :row="row"
      ...
      <div v-if="league.promotionPlayoff.length" class="space-y-2">
        ...
        <LazyUPageGrid>
          <TeamCard
            v-for="(row, index) in league.promotionPlayoff" :key="getRowKey('pr', row, index)"
      ...
      <div v-if="league.relegated.length || league.forcedRelegations.length" class="space-y-2">
        ...
        <LazyUPageGrid>
          <TeamCard
            v-for="(row, index) in getCombinedRelegations(league)" :key="getRowKey('r', row, index)"
      ...
      <div v-if="league.relegationPlayoff.length" class="space-y-2">
        ...
        <LazyUPageGrid>
          <TeamCard
            v-for="(row, index) in league.relegationPlayoff" :key="getRowKey('rr', row, index)"
            :row="row" :league-organization="league.organization" highlight-color="warning"
          />
        </LazyUPageGrid>
      </div>
